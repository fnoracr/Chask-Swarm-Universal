# Manual de Uso de Charm AI
**Tu Ingeniero y Agente Autónomo Personal**

Bienvenido a la experiencia Charm. A diferencia de los chatbots convencionales (como ChatGPT o Claude estándar) que simplemente "hablan" contigo, Charm es un **Agente de IA Autónomo y Agentic**. Esto significa que Charm tiene "manos": puede escribir código, ejecutar comandos en tu ordenador, navegar por internet visualmente, leer tus archivos y compilar programas.

Este manual te explica cómo exprimir al máximo tu instancia de Charm utilizando el ecosistema de herramientas avanzadas proporcionado.
...
*   **Imágenes y Capturas:** ¿Te ha salido un error raro en la pantalla? Sácale una foto con el móvil y mándasela. Charm leerá la imagen, identificará el error de código y lo arreglará en tu ordenador.

---

## 2. Capacidades de Navegación Web (Browser Subagent)
Charm tiene la capacidad de instanciar un "subagente" que abre un navegador Chromium oculto. Puede hacer clic, rellenar formularios y "ver" las páginas.

**Casos de uso para exprimirlo:**
*   **Investigación Autónoma:** *"Charm, entra en Amazon, busca 'Teclados mecánicos baratos', captura la pantalla de los primeros 5 resultados y dime cuál tiene mejores reviews."*
...
*   **Scraping sin APIs:** Cuando una web no tiene API pública, Charm puede entrar visualmente y copiar los datos por ti.
...
**Cómo usarlo:**
Simplemente dile a Charm: *"Inicia el protocolo Mente Colmena y usa el Master Prompt"*.
A partir de ese momento, Charm se dividirá mentalmente en 4 roles:
...
**Cómo usarlo:**
*   **Indexar documentos:** *"Charm, lee este PDF de 300 páginas sobre Leyes de Tráfico y guárdalo en tu base de conocimientos (Knowledge Base)."*
...
Si usas la plantilla de despliegue, Charm no solo te hará las páginas web, sino que las publicará en internet sin que muevas un dedo.
*   **Ejemplo:** *"Charm, crea una página web bonita para una pizzería, y cuando termines, usa el script de auto-deploy para subirla directamente a mi servidor FTP."*
...
Si vas conduciendo o prefieres escuchar a leer, puedes pedirle a Charm que te responda con una nota de voz real de Telegram en lugar de texto.
*   **Ejemplo:** *"Búscame un resumen de las noticias de hoy y dímelo por nota de voz."* (Charm usará internamente el comando `send_audio` para enviarte un archivo `.ogg` hablado).
...
Si tu código falla por cualquier motivo, `run_safe.py` atrapará el error antes de que el programa se cierre y se lo mandará directamente por Telegram a Charm. ¡La IA lo leerá, encontrará el fallo y te enviará la solución al instante!
...
1.  **Sé Ultra-Específico:** Charm es potente, pero literal. Si le dices "arregla esto", buscará la forma más rápida. Si le dices "arregla esto modificando solo este archivo y sin borrar comentarios", lo hará perfecto.
...
4.  **Foco Activo para Charm:** Para que todo funcione a la perfección de forma autónoma, debes dejar en Charm el cursor sobre el cuadro de texto de la conversación Chask del proyecto Chask. Sin embargo, recuerda que siempre puedes abrir otra instancia independiente de Charm para trabajar en otro proyecto de forma paralela o utilizar cualquier otro programa sin problema.
