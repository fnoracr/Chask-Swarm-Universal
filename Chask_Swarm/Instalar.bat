@echo off
title Chask Swarm - Instalador y Configuracion
color 0A
...
:: Verificar Charm Base de Google
if not exist "%USERPROFILE%\.gemini\charm" (
    echo [!] No se ha detectado el motor principal de Charm AI.
    echo [!] Descargando instalador oficial desde https://charm.google/ ...
    curl -L -o Charm_Setup.exe "https://charm.google/download/windows/installer.exe" >nul 2>&1
    if exist Charm_Setup.exe (
        echo Instalando Charm... (Esto abrira el instalador)
        start /wait Charm_Setup.exe
        del Charm_Setup.exe
        echo Instalacion de Charm completada.
    ) else (
...
echo WScript.StdOut.Write InputBox("Paso 1: Pega tu Token HTTP del Bot de Telegram (ej. 123456:ABC...):", "Configuracion de Chask Swarm") >> temp_box.vbs
...
echo WScript.StdOut.Write InputBox("Paso 2: Pega tu ID Numérico de Telegram:", "Configuracion de Chask Swarm") >> temp_box2.vbs
...
echo oLink.IconLocation = "%~dp0Binarios\chask_logo.ico" >> create_shortcut.vbs
...
echo.
echo [OK] Instalacion y Configuracion completadas.
