...
- La terminación notifica a Nora automáticamente (sin tokens).
...
def _ack_web(user_text: str):
    ...
    try:
        resp_file = os.path.join(BASE_DIR, "Advanced_Tools", "Colas_Mensajes", "web_responses.json")
        data = []
        ...
        if os.path.exists(QUEUE_FILE):
            ...
            if pending:
                ...
                for item in pending:
                    ...
                    if not ok:
                        # 3. WAKEUP_PING para notificar a Nora (si la inyección falla y Nora la está ejecutando localmente)
