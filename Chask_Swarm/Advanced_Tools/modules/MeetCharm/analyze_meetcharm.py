...
# 1. Ver docker-compose de MeetCharm
print("=== docker-compose.yml de MeetCharm ===")
run(ssh, "cat /opt/MeetCharm/docker-compose.yml 2>/dev/null || cat /root/proyecto_norameet/docker/docker-compose.yml 2>/dev/null", wait=5)

# 2. Ver el .env de MeetCharm (ICE config)
print("\n=== .env de MeetCharm ===")
run(ssh, "find /root/proyecto_norameet /opt/MeetCharm -name '.env' -o -name '*.env' 2>/dev/null | xargs cat 2>/dev/null | head -60", wait=8)
...
# 4. Ver el main.py de la API de MeetCharm
print("\n=== main.py de MeetCharm API ===")
