...
class ProtocoloOrestes:
    ...
    def forge_script(self, task_description, filename):
        ...
        if not code_result:
            self.log("Alerta de Créditos/Red: Ninguna IA externa disponible. El Orquestador (Enjambre/Antigravity) debe asumir el rol de forjador simulando al agente.")
            return False
    ...
    def audit_code(self, filename):
        ...
        if not audit_result:
            self.log("Alerta de Créditos/Red: Fallo en la conexión de auditoría. El Orquestador (Enjambre/Antigravity) debe asumir el rol de auditor revisando críticamente el código.")
            return False
