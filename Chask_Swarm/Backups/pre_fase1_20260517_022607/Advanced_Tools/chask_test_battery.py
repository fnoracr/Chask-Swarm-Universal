...
def capture_system_state():
    ...
    try:
        ...
        w = auto.WindowControl(searchDepth=1, SubName='Antigravity')
        state["antigravity"] = "open" if w.Exists(1) else "closed"
    except: state["antigravity"] = "unknown"
    return state
...
def test_window_find(i):
    """Test: encontrar ventana Antigravity."""
    try:
        from computer_use import ComputerUse
        cu = ComputerUse()
        found = cu.find_window("Antigravity")
        return found, "Encontrada" if found else "No encontrada"
    except Exception as e: return False, str(e)[:100]

def test_injector_find(i):
    """Test: localizar chat input en Antigravity (sin inyectar)."""
    try:
        import uiautomation as auto
        w = auto.WindowControl(searchDepth=1, SubName='Antigravity')
...
ALL_TESTS = {
    ...
    "UIA: Encontrar Antigravity": test_window_find,
...
<p>Antigravity: <span class="badge {'badge-ok' if ist.get('antigravity')=='open' else 'badge-warn'}">{ist.get('antigravity','?')}</span></p>
</div>
...
<p>Antigravity: <span class="badge {'badge-ok' if fst.get('antigravity')=='open' else 'badge-warn'}">{fst.get('antigravity','?')}</span></p>
</div>
</div>
...
def send_telegram_summary(data):
    ...
    msg = "\n".join(lines)
    try:
        tg = os.path.join(BASE_DIR, "antigravity_telegram.py")
