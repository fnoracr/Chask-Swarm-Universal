...
<div id="toolbar">
  <h1>{title}</h1>
  <span id="tip">Arrastra los grupos por su titulo | Scroll = zoom | Clic = detalle</span>
  ...
  <div class="controls">
    ...
    <button onclick="loadPositions()">Cargar</button>
  </div>
</div>
<div id="toast"></div>
...
const savedPos=JSON.parse(localStorage.getItem('chask_canvas_positions')||'null');
...
function savePositions(){{
  ...
  showToast('Canvas descargado + posiciones copiadas al portapapeles');
}}
function loadPositions(){{
  const input=prompt('Pega aqui el JSON de posiciones guardado:');
  if(!input)return;
  try{{
...
def chask_swarm_architecture():
    ...
    groups = [
        {"id":"boot","label":"Arranque","color":"#f59e0b","x":30,"y":30,"nodes":[
        ...
        {"id":"comms","label":"Comunicaciones","color":"#3b82f6","x":420,"y":30,"nodes":[
            ...
            {"id":"tg_send","label":"antigravity_telegram.py","desc":"Utilidad CLI para enviar mensajes a Telegram. Usado por Enjambre y otros scripts para envío unidireccional. No es un daemon.","icon":"📤"},
            {"id":"channel","label":"channel_adapter.py","desc":"Router multicanal genérico. Adaptadores para Discord (webhook), Slack, Teams (Adaptive Cards), Monday.com (GraphQL), n8n (webhooks).","icon":"🔀"},
        ]},
        {"id":"queue","label":"Cola de Mensajes","color":"#8b5cf6","x":420,"y":310,"nodes":[
            {"id":"input_q","label":"Colas_Mensajes/input_queue.json","desc":"Cola principal con garantia de entrega. Los canales escriben con status 'pending'. El daemon inyecta y marca 'injected'. Enjambre BORRA cada mensaje solo cuando termina de procesarlo. Si se cuelga, los no procesados persisten para el siguiente arranque.","icon":"📋"},
            {"id":"pending","label":"Colas_Mensajes/pending_messages.json","desc":"Historial de mensajes para el dashboard. Se mantiene para auditoría y contexto visual.","icon":"📑"},
        ]},
        {"id":"ide","label":"IDE Antigravity","color":"#10b981","x":810,"y":30,"nodes":[
            {"id":"antigravity","label":"Antigravity IDE (Enjambre)","desc":"Interfaz principal donde Enjambre opera. Los daemons inyectan mensajes aquí via pyautogui+pyperclip (ctrl+v). Enjambre (Claude) responde desde aquí.","icon":"🖥️"},
            {"id":"inject","label":"inject_to_antigravity.py","desc":"Módulo de inyección universal. Busca la ventana de Antigravity, obtiene foco con truco Alt, pega con Ctrl+V. 3 reintentos con verificación de foco.","icon":"📌"},
        ]},
        {"id":"daemons","label":"Daemons Secundarios","color":"#06b6d4","x":30,"y":280,"nodes":[
            {"id":"watchdog","label":"Advanced_Tools/process_watchdog.py","desc":"CENTINELA: Vigila cada 30s que todos los daemons estén vivos. Si alguno cae, INYECTA aviso directamente en el IDE. Si Enjambre/IDE está caída, la REINICIA. Primer proceso en cerrarse al cerrar IDE.","icon":"👁️"},
            {"id":"shutdown","label":"Advanced_Tools/shutdown_cleanup.py","desc":"Detecta cuando se cierra el IDE. Primero cierra process_watchdog, luego mata el resto de daemons ordenadamente.","icon":"🛑"},
        ...
        {"id":"ai","label":"Inteligencia Artificial","color":"#ec4899","x":810,"y":280,"nodes":[
            ...
            {"id":"hive","label":"hive_mind_executor.py","desc":"Protocolo Mente Colmena. Alpha (planifica) -> Beta (investiga) -> Gamma (codifica) -> Delta (audita). Delegación multi-modelo.","icon":"🐝"},
            ...
            {"id":"evomem","label":"evolutionary_memory.py","desc":"Aprendizaje continuo: aprende de correcciones de Fernando Y de sus propios aciertos/errores del día a día. Persiste lecciones en Qdrant para no repetir fallos.","icon":"🧪"},
        ]},
        {"id":"mcp","label":"MCPs e Integraciones","color":"#14b8a6","x":1200,"y":30,"nodes":[
            ...
            {"id":"mcp_config","label":"Configuracion/mcp_config.json","desc":"Configuración de servidores MCP registrados (propios y externos). Define comandos, args y rutas.","icon":"⚙️"},
        ]},
        {"id":"security","label":"Seguridad","color":"#ef4444","x":1200,"y":300,"nodes":[
            ...
            {"id":"authusers","label":"Configuracion/authorized_users.json","desc":"Lista de usuarios autorizados por plataforma (Discord ID, Telegram ID) con roles (Master/Admin).","icon":"👤"},
        ]},
        {"id":"infra","label":"Infraestructura","color":"#f97316","x":1200,"y":560,"nodes":[
        ...
        {"id":"data","label":"Datos Persistentes","color":"#84cc16","x":420,"y":550,"nodes":[
            ...
            {"id":"channels_cfg","label":"Configuracion/channels_config.json","desc":"Configuración de todos los canales: Discord webhook/bot token, Teams webhook, Monday API, n8n.","icon":"⚙️"},
        ]},
        {"id":"integrations","label":"Integraciones Externas","color":"#a855f7","x":30,"y":750,"nodes":[
            ...
            {"id":"int_telegram","label":"Telegram","desc":"Bot Telegram integrado en unified_daemon (long polling). Envio via antigravity_telegram.py. Canal principal de comunicacion con Fernando.","icon":"✈️"},
        ...
        {"id":"future","label":"Desarrollo Futuro","color":"#facc15","x":810,"y":750,"nodes":[
            {"id":"fut_multi","label":"Multiusuario en equipo","desc":"Multiples usuarios operando sus propias instancias de Enjambre en un mismo equipo. Perfiles independientes, memoria separada, sesiones simultaneas con aislamiento de contexto.","icon":"👥"},
            {"id":"fut_wifi","label":"Red de Enjambres via WiFi","desc":"Enjambres domesticos conectados en red local. Multiples PCs con Enjambre se descubren y colaboran via WiFi, compartiendo tareas, memoria y recursos de forma descentralizada.","icon":"📡"},
            {"id":"fut_global","label":"Red de Colmenas Mundial","desc":"Red global de enjambres interconectados via servidor central o Discord. Colmenas de distintos usuarios colaboran a escala mundial, compartiendo conocimiento y capacidades distribuidas.","icon":"🌍"},
        ]},
    ]
    ...
    connections = [
        ...
        {"from":"inicio","to":"antigravity","label":"Abre IDE","color":"#f59e0b"},
        ...
        {"from":"inject","to":"antigravity","label":"Ctrl+V","color":"#10b981"},
        {"from":"antigravity","to":"tg_send","label":"Envia/Recibe","color":"#10b981","bidi":True},
        {"from":"antigravity","to":"channel","label":"Broadcast","color":"#10b981","bidi":True},
        ...
        {"from":"watchdog","to":"antigravity","label":"Reinicia si caida","color":"#06b6d4","dashed":True},
        ...
        {"from":"antigravity","to":"llm","label":"Delega","color":"#ec4899","dashed":True},
        {"from":"llm","to":"hive","label":"Colmena","color":"#ec4899","dashed":True},
        {"from":"antigravity","to":"elektra","label":"Activa autonomo","color":"#ec4899","dashed":True},
        {"from":"antigravity","to":"qdrant","label":"Guarda/Busca","color":"#ec4899","bidi":True},
        ...
        {"from":"antigravity","to":"evomem","label":"Aprende","color":"#ec4899","bidi":True},
        {"from":"antigravity","to":"audit","label":"Pre-comando","color":"#ef4444"},
        {"from":"antigravity","to":"mcp_server","label":"Expone tools","color":"#14b8a6","bidi":True},
        {"from":"antigravity","to":"mcp_client","label":"Consume tools","color":"#14b8a6","bidi":True},
