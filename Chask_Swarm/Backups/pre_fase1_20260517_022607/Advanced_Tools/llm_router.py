...
def load_context() -> str:
    """
    Construye un system prompt rico con la memoria, alma y directivas del sistema.
    Se inyecta en TODAS las llamadas a IAs gratuitas para que tengan contexto.
    """
    sections = []
    files = [
        ("soul.md",         "## Identidad y Personalidad"),
        ("directives.md",   "## Directivas Activas"),
        ("memory.md",       "## Memoria de Trabajo Actual"),
        ("projects_memory.md", "## Proyectos en Curso"),
    ]
    for fname, header in files:
    ...
    header = (
        "Eres Antigravity, asistente IA del ecosistema Chask Swarm de Fernando Enjambre.\n"
...
def complexity_score(prompt: str, cfg: dict) -> tuple[int, str]:
    ...
    >= 60 → Antigravity. < 60 → Pool gratuito.
    """
    p = _norm(prompt)
    score = 0
    reasons = []
    ...
    # Si el mensaje EMPIEZA por un nombre de la IA -> Antigravity siempre
    ...
    # ── 1. Acciones que requieren herramientas del sistema (siempre Antigravity)
    ACTION_KEYWORDS = [
    ...
    for kw in ACTION_KEYWORDS:
        if _norm(kw.strip()) in p:
            ...
            break  # uno basta para ser Antigravity
...
def is_complex(prompt: str, cfg: dict) -> bool:
    ...
    print(f"[Router] Complejidad: {score}/100 ({reason}) -> {'Antigravity' if verdict else 'Pool gratuito'}")
    return verdict
...
def route(prompt: str, system_prompt: str = "", source: str = "", force_free: bool = False) -> dict:
    """
    ...
    init_privacy() # Inicializar solo cuando se necesite
    if PRIVACY_SHIELD_ACTIVE and privacy_engine:
        original_prompt = prompt
        prompt = privacy_engine.anonymize(prompt)
        if prompt != original_prompt:
            print("[Router] Privacy Shield: Datos sensibles anonimizados automáticamente.")
    ...
    # -- PRIMERA BARRERA: Nombres directos -> Antigravity SIEMPRE --
    ...
    if first_token in direct_names:
        return {
            "engine": "antigravity",
            "response": None,
            "reason": f"Nombre directo '{first_token}' -> Antigravity"
        }

    # Tareas complejas → Antigravity
    if not force_free and is_complex(prompt, cfg):
        score, reason = complexity_score(prompt, cfg)
        return {
            "engine": "antigravity",
            "response": None,
            "reason": f"Tarea compleja ({score}/100: {reason}) -> Antigravity"
        }
    ...
    if not provider:
        return {
            "engine": "antigravity",
            "response": None,
            "reason": "Sin proveedores disponibles -> Antigravity"
        }
