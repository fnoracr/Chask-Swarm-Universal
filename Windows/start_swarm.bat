...
REM Configurar Workspace de [Nombre_IA] automáticamente
...
SET AUDIT_LOG="%BASE%\System_Logs\swarm_power_audit.log"
...
echo [%date% %time%] [BATCH] Levantando IDE Antigravity (Instancia Unica [Nombre_IA])... >> %AUDIT_LOG%
set "LAUNCH_IDE_VBS=%BASE%\_launch_ide_tg.vbs"
> "%LAUNCH_IDE_VBS%" echo Set WshShell = CreateObject("WScript.Shell")
>> "%LAUNCH_IDE_VBS%" echo WshShell.Run """" ^& "%IDE%" ^& """ ""%BASE%\[Nombre_IA]""", 1, False
